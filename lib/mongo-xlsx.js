    ...
    for (index = 0; index < mongoModel.length; index+=1) {
      ...
      for (var i = 0; i < access.length; i+=1) {
        if (typeof aCell === 'undefined' || aCell === null) {
          ...
          if (isOriginal && aCell[access[i]] && // (i < (access.length - 1)) &&
            (aCell[access[i]].constructor === Array || aCell[access[i]].constructor === Object)) {
            isOriginal = false;
            aCell = JSON.parse(JSON.stringify(aCell[access[i]]));
          } else {
            if (typeof aCell[access[i]] === 'object') {
